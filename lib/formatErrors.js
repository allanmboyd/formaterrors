...
exports.formatStack = function (error, stackFormat) {
    ...
    var stack = getMessages(enhanced).join(" ") + "\n";
 ...
 */
exports.highlightAssertionError = function (assertionError, messageLineHighlights, stackHighlights, stackPatterns, inclusive) {
    if (!isActualExpectedError) {
        return exports.applyHighlightsStackHighlights(assertionError.stack, messageLineHighlights, stackHighlights,
            stackPatterns, inclusive);
    }
};
...
function isActualExpectedError(error) {
    return error.stack.expected && error.stack.actual;
}
 ...
 * multiple items if the message part consists of mulitple lines.)
 *
